...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __("Les Points d'interets") }}  :   <a style="color:green;" href="{{ route('interest.index') }}">Créer un nouveau point d'interet</a>
        </h2>
    </x-slot>
            ...
            <div class="grid md:grid-cols-1 gap-4">
                <div class="lg:col-span-2 md:col-span-2 bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="grid lg:grid-cols-1 sm:grid-cols-1 md-cols-1 lg:gab-4 p-8">
                        <form action="{{ route('interest.update')}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-6">
                                <label for="alentour_name"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Point D'interet</label>
                                <input type="text" id="interest" name="interest" value="{{ $oneinterest->title }}"
                            ...
                            <div class="mb-6">
                                ...
                                <select id="category" name="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    ...
                                    @foreach ($categories as $category)
                                         <option {{ $category->id == $oneinterest->category_id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->title }}</option>
                                    @endforeach
                                 </select>
