...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __("Mise à jour de Livre d'Or | ") }} <a style="color: red" href="{{ route('wifi.index') }}">Créer un nouveau</a>
        </h2>
    </x-slot>
            ...
            <div class="grid md:grid-cols-1 gap-4">
                <div class="lg:col-span-2 md:col-span-2 bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="grid lg:grid-cols-1 sm:grid-cols-1 md-cols-1 lg:gab-4 p-8">
                        <form action="{{ route('wifi.updates')}}" method="POST">
                            @csrf
                            <div class="mb-6">
                                <label for="wifi_name"
                                ...
                                <input type="text" id="wifi_name" name="wifi_name"  value="{{ $onewifi->wifi_name }}"
                            ...
                            <select id="type_cryptage_wifi" name="type"
                                ...
                                <option {{ $onewifi->type == 'WEP' ? 'selected' : '' }}>WEP</option>
                                <option {{ $onewifi->type == 'WPA' ? 'selected' : '' }}>WPA</option>
                                <option {{ $onewifi->type == 'WPA 2' ? 'selected' : '' }}>WPA 2</option>
                            </select>
                            </div>
                            ...
                            <div class="mb-6">
                                <label for="password_wifi"
                                    ...
                                    de passe</label>
                                <input type="text" id="password_wifi" placeholder="Wer342jO9 Ex" name="wifi_pass" value="{{ $onewifi->code }}"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light">
                            </div>
                            <input type="hidden" value="{{ $onewifi->id }}" name="wifi_id">
                            <button type="submit"
